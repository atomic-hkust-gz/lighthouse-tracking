...
int mote_main(void) {
  ...
  sctimer_set_callback(0,cb_compare);
  sctimer_setCompare(0,sctimer_readCounter()+SAMPLE_PERIOD);
...
leds_error_on();
 ...
 // ts_nrf_gpio_cfg_output(SYNC_OUT_PIN);
  ...
  delay_ms(10);
  //nrf_gpio_cfg_output(0,TS4231_GPIO_PIN);
  leds_flag_on();
...
void cb_compare(void) {
  ...
  // schedule again
  sctimer_setCompare(2,sctimer_readCounter() + SAMPLE_PERIOD);
