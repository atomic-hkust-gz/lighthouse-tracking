...
\brief This program shows how to use lighthouse to obtain data and Bluetooth to transmit coordinates.
...
\author Zhoutao Bi <bizhoutao.terry@gmail.com>, july 2025
...
void assemble_ibeacon_packet(void) {
    ...
    app_vars.packet[i++]  = data_xy[0];
    app_vars.packet[i++]  = data_xy[1];
    app_vars.packet[i++]  = data_xy[2];
    app_vars.packet[i++]  = data_xy[3];

    app_vars.packet[i++]  = data_xy[4];
    app_vars.packet[i++]  = data_xy[5];
