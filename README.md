# introuduction
The branch include three parts.
1. lighouse_host
   
    The lighouse_host can realize functions such as serial port display, coordinate trajectory drawing, and coordinate calibration and conversion. 
    
    For more details, please refer to the readme in the specific folder

3. lighthouseTag_nrf52833_PCB
   
    please import it using the web version of LCeda pro to open it.

   For more details, please refer to the readme in the specific folder

5. nrf52833_radio_ble_lighthouse
   
    This program shows how to use lighthouse to obtain data and Bluetooth to transmit coordinates.

   For more details, please refer to the readme in the specific folder


# author 
Zhoutao Bi <bizhoutao.terry@gmail.com>  july 2025

