...
void ts4231_init(){
  ...
  while ( ts4231_configDevice() != 0x04){
  Config_OK = 0;
...
bool ts4231_waitForLight(uint16_t light_timeout) {
  ...
  if (ts4231_checkBus() == S0_STATE) {
    ...
    while (exit == false) {
      if (ts_nrf_gpio_read_input(ts4231_var.D_pin) > 0) {
        while (exit == false) {
          if (ts_nrf_gpio_read_input(ts4231_var.D_pin) == 0) {
            exit = true;
            light = true;
...
bool ts4231_goToSleep(){
  ...
  if (ts4231_var.configured == false)  sleep_success = false;
  else {
    switch (ts4231_checkBus()) {
      ...
      case WATCH_STATE:
        ...
        ts4231_pinMode(TS4231_N1_E_PIN, MODE_INPUT);
        delay_us(BUS_DRV_DLY);
        if (ts4231_checkBus() == SLEEP_STATE) sleep_success = true;
...
bool ts4231_goToWatch(void){
  ...
  if (ts4231_var.configured == false)  watch_success = false;
  else {
    switch (ts4231_checkBus()) {
      case S0_STATE:
        watch_success = false;
      ...
      case SLEEP_STATE:
        ...
        if (ts4231_checkBus() == WATCH_STATE) watch_success = true;
...
switch(mode){
    case 0:
    ts_nrf_gpio_cfg_input(pin_number);break;
    case 1:
    ts_nrf_gpio_cfg_output(pin_number);break;
    }
}
...
void ts4231_writeConfig(uint16_t config_val){
  ...
  for (uint8_t i = 0; i < 15; i++) {
    ...
    if ((config_val & 0x8000) > 0) ts4231_digitalWrite(TS4231_N1_D_PIN, OUTPUT_HIGH);
    else ts4231_digitalWrite(TS4231_N1_D_PIN, OUTPUT_LOW);
    delay_us(BUS_DRV_DLY);
    ts4231_digitalWrite(TS4231_N1_E_PIN, OUTPUT_HIGH);
    delay_us(BUS_DRV_DLY);
    ts4231_digitalWrite(TS4231_N1_E_PIN, OUTPUT_LOW);
