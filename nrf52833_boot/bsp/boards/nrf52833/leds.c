...
#define LED_SYNC           NRF_GPIO_PIN_MAP(0,17) // p0.17
#define LED_DEBUG          NRF_GPIO_PIN_MAP(0,16) // p0.16


#define LED_FLAG           NRF_GPIO_PIN_MAP(0,2)
...
void    leds_init(void) {
    ...
    nrf_gpio_cfg_output(0,LED_FLAG);
    leds_all_off();

}


void    leds_flag_off(void) {

    NRF_P0->OUTCLR =  1 << LED_FLAG;
}


void    leds_flag_on(void) {
    
     NRF_P0->OUTSET =  1 << LED_FLAG;
}
